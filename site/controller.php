...
class AdvancedOpenPortalController extends JControllerLegacy
{
    ...
    public function toggleCaseStatus()
    {
        ...
        JFactory::getApplication()->redirect(
            JURI::base() . "?option=com_advancedopenportal&view=showcase&id=" . $_REQUEST['case_id']
        );
    }
    ...
    function newcase()
    {
        ...
        JFactory::getApplication()->redirect(
            JURI::base() . "?option=com_advancedopenportal&view=showcase&id=" . $case->id
        );
    }
    ...
    function addupdate()
    {
        ...
        $caseUpdate = SugarUpdate::fromScratch(
            array(
                'name' => $description,
        ...
        $caseUpdate = $caseUpdate->save();

        $caseUpdate->loadContact();

        $caseUpdate->loadDisplayData();

        $jsonResponse = $caseUpdate->toJson();
    ...
    function create()
    {
        ...
        if (isset($_REQUEST['sug']) && $_REQUEST['sug'] != '') {
            ...
            if (!empty($contacts['data'])) {
                ...
                $autoregister = isset($options['autoregister']) ? $options['autoregister'] : $params->get(
                    'autoregister',
                    1
                );
                ...
                echo json_encode(array("error" => "Unable to retrieve user"));
            }
        } else {
